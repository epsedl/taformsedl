...
router.post('/bulk-resend-activation', auth, async (req, res) => {
  ...
  try {
    ...
    for (const emp of rows) {
      ...
      const activationLink = `${process.env.FRONTEND_URL || 'https://taformsed.netlify.app'}/activate-account?token=${token}`;
      try {
...
router.post('/admin/send-reset-password-link', auth, async (req, res) => {
  ...
  try {
    ...
    const resetLink = `${process.env.FRONTEND_URL || 'https://taformsed.netlify.app'}/activate-account?token=${token}`;
    await EmailService.sendEmail(
      email,
